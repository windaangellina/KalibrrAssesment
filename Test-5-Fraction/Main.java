...
public class Main {
    ...
    public static void computeFraction() throws FileNotFoundException {
        // Scanner input = new Scanner(System.in);
        // Scanner input = new Scanner(new File("./test.in"));
        File file = new File("D:\\FileKuliah\\Kalibrr\\test.in");
        Scanner input = new Scanner(new BufferedReader(new FileReader(file)));

        int testCase = input.nextInt();

        ArrayList<String> listResultContest = new ArrayList<>();
        ArrayList<String> listResultConquer = new ArrayList<>();
        ...
        for (int i = 1; i <=testCase; i++) {
            ...
            for(Map.Entry<String,Integer> entry : fractions.entrySet()) {
                String key = entry.getKey();
                Integer val = entry.getValue();
                itemConquer += key+" "+ val + "_";                
            }
            listResultContest.add("contested "+Conquer);
            listResultConquer.add(itemConquer);
        }


        for (int i = 0; i < testCase; i++) {
            String[] arrConquered = listResultConquer.get(i).split("_");

            System.out.println("Case " + (i + 1) + ":");
            for (String itemConquer : arrConquered) {
                System.out.println(itemConquer);
            }
            System.out.println(listResultContest.get(i));
        }
    }
